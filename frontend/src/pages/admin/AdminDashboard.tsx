...
export default function AdminDashboard() {
  ...
  const cards = [
    ...
    { label: 'Visiteurs aujourd\'hui', value: stats.todayVisitors, sub: `${stats.weekVisitors} cette semaine`, icon: Eye, color: '#9c27b0' },
    { label: 'Newsletter', value: stats.newsletterSubs, sub: 'abonnés actifs', icon: Mail, color: '#c9a96e' },
  ];
  ...
  return (
    <div>
      <h1 style={{ fontFamily: 'Georgia, serif', fontSize: '1.8rem', fontWeight: 400, color: '#1a1a1a', marginBottom: '2rem' }}>Tableau de bord</h1>
        ...
        <div style={{ backgroundColor: 'white', border: '1px solid #e8d5b7', padding: '1.5rem' }}>
          ...
          {stats.topItems?.slice(0, 8).map((item: any) => (
            <div key={item.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', borderBottom: '1px solid #f0ece6' }}>
              <div>
                <p style={{ fontFamily: 'Helvetica Neue, Arial, sans-serif', fontSize: '0.78rem', color: '#1a1a1a' }}>{item.title}</p>
            ...
            {(stats.dailyVisitors || []).slice(-30).map((d: any, i: number) => {
              ...
              return (
                <div key={i} title={`${d.day}: ${d.count} visiteurs`}
                  style={{ flex: 1, backgroundColor: '#c9a96e', height: `${h}%`, minWidth: '6px', opacity: 0.7 + 0.3 * (d.count / (max || 1)) }} />
              );
            })}
          </div>
        </div>
