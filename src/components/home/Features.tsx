    ...
    <section className="py-16 bg-[#fafaf8]">
      <div className="max-w-6xl mx-auto px-5 lg:px-8">
        <div className="flex items-end justify-between mb-8">
          <div>
            <p className="text-[10px] text-[#aaaaaa] uppercase tracking-widest mb-1.5 font-medium">Plateforme</p>
            <h2 className="text-base font-bold text-[#0f0f0f] tracking-tight">{t('features.title')}</h2>
          </div>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-px bg-black/6 border border-black/6 rounded-2xl overflow-hidden">
          {features.map((f) => (
            <div key={f.title} className="group bg-[#fafaf8] hover:bg-white transition-colors p-5 relative">
              <div className="flex items-start justify-between mb-3">
                ...
                <span className="text-[9px] font-mono text-black/15 font-bold mt-1">{f.n}</span>
              </div>
              <h3 className="text-[#0f0f0f] font-semibold text-xs mb-1">{f.title}</h3>
              <p className="text-[#6b6b6b] text-[11px] leading-relaxed">{f.description}</p>
              <div className="absolute bottom-0 left-0 w-0 h-[1.5px] group-hover:w-full transition-all duration-500" style={{background:f.color}} />
            </div>
          ))}
        </div>
      </div>
